...
describe('Transform template cache', () => {
    ...
    it('should fetch and cache template text with validators', async () => {
        ...
        const result = await fetchTransformTemplate(
            storage,
        ...
        expect(storage.put).toHaveBeenCalledWith(
            expect.stringContaining('transform_template_'),
    ...
    it('falls back to cached template when revalidation fails before max age', async () => {
        ...
        try {
            ...
            expect(warnSpy).toHaveBeenCalledWith(
                '[TemplateCache] Template fetch failed with HTTP 503, falling back to cached template'
            );
        } finally {
            warnSpy.mockRestore();
        }
    ...
    it('throws when remote fetch fails and no cached template is available', async () => {
        ...
        await expect(
            fetchTransformTemplate(storage, 'https://example.com/template.yaml')
        ).rejects.toThrow('Template fetch failed: HTTP 503');
    });
});
